...
function PreventDeleteText(className, selectedDatePart) {
    window.onkeydown = function (event) {
        if (event.which == 8 || event.which == 46) {
            ...
            if ($(className).val() == '/undefined/undefined')
                $(className).val('dd/mm/yyyy')
...
function DeleteKeyPress(e, className, selectedDatePart) {
    ...
    if ($(className).prop('required'))
        CheckPattern(className);
    else
        $(className).removeAttr("pattern");
}
...
function InputEvent(className, selectedDatePart, calendarDisplayed) {
    CloseCalendar(className, calendarDisplayed);
    if ($(className).val().length == 10)
        SetMaxAndMinDayAndMonth(className);
    CheckDate(className, selectedDatePart);
    if ($(className).prop('required'))
...
function MouseClickEvent(e, className, selectedDatePart, calendarDisplayed) {
    CloseCalendar(className, calendarDisplayed);
...
function CheckPattern(className) {
    if (typeof $(className).val() !== 'undefined') {
        var date = $(className).val().split("/");
        if (!isNaN(date[0]) && !isNaN(date[1]) && !isNaN(date[2]))
            $(className).removeAttr("pattern");
        else
            $(className).prop("pattern", "\d{1,2}/\d{1,2}/\d{4}");
    }
}

function CloseCalendar(className, calendarDisplayed) {
    if ($(calendarDisplayed).val() == 1)
        $(className).next("span").find("button").trigger("click");
}
...
function DatePicker(className, calendarDisplayed, selectedDatePart) {
    ...
    $(className).mousedown(function (e) {
        MouseClickEvent(e, className, selectedDatePart, calendarDisplayed);
    });
    ...
    $(className).on("input", function () {
        InputEvent(className, selectedDatePart, calendarDisplayed);
    });
